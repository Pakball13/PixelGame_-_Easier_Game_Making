# pg_interpreter.py - FIXED VERSION! Now with movement, non-square sprites, set, reverse, and more
# Run: python pg_interpreter.py yourgame.pg
...
class PGGame:
    def __init__(self, width=800, height=600, world_width=2500, fps=60, title="PixelGame"):
    ...
    def update_physics(self):
        for name, s in list(self.sprites.items()):
            ...
            if player_rect and r.colliderect(player_rect) and name != 'player' and name != 'mario' and name != 'bird':
    ...
    def exec_cmd(self, keys, line):
        for sub_line in line.split(';'):
            ...
            cmd = words[0].lower()
            if cmd == 'create':
                nwords = sub_line.split()
                name = nwords[1]
                i = 2
                if nwords[i] != 'at': continue
                i +=1
                x_str = nwords[i].rstrip(',')
                i +=1
                y_str = nwords[i]
                x = float(x_str)
                y = float(y_str)
                ...
                if is_plat:
                    ...
                    if nwords[i] == 'size':
                        ...
                        if nwords[i] == 'height':
                            ...
                            h = float(nwords[i])
                            i +=1
                if i < len(nwords) and nwords[i] == 'color':
                    i +=1
                    color = nwords[i]
                if is_plat:
